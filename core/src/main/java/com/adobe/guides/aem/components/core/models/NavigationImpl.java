...
public class NavigationImpl extends AbstractComponentImpl implements Navigation {
     ...
     */
    public static final String RESOURCE_TYPE = "guides-components/components/navigation";
    ...
    private static final String PN_TEMPLATE_NAVIGATION_ROOT = "templateNavigationRoot";
    ...
    private Page findParentByTemplate(final Page page, final String templatePath) {

        if (StringUtils.isBlank(templatePath)) { return null; }

        Page parent = page;
        while (parent != null) {
            parent = parent.getParent();
            if (templatePath.equalsIgnoreCase(parent.getTemplate().getPath())) {
               return parent;
            }
        }
        return null;
    ...
    private Page getNavigationRoot() {
        if (this.navigationRootPage == null) {
            ...
            String templateNavigationRootPath = this.resource.getValueMap().get(PN_TEMPLATE_NAVIGATION_ROOT, String.class);
            Page rootPageWithTemplate = findParentByTemplate(currentPage, templateNavigationRootPath);
            Page rootPage = (rootPageWithTemplate != null)
                ? rootPageWithTemplate
                : pageManager.getPage(navigationRootPath);
     ...
     * A page is selected if it is the current page:
     * @param page The page to check.
     * @return True if the page is selected, false if not.
     */
    private boolean isCurPage(@NotNull final Page page) {
